...
class QueryBase(object):
    ...
    def get_full_user_info(user_id):
        ...
        user_info_request = requests.get(
            url='{0}/userinfo/by_id/{1}'.format(METADATA_ENDPOINT, user_id),
            headers=header_list
        )
        ...
        prop_list = QueryBase._get_user_lookups(
            '{0}/proposalinfo/by_user_id/{1}'.format(METADATA_ENDPOINT, user_id), header_list)
        # get available instruments for this user
        inst_list = QueryBase._get_user_lookups(
            url='{0}/instrumentinfo/by_user_id/{1}'.format(METADATA_ENDPOINT, user_id), header_list=header_list)
